...
class UserTestHandler {
	...
	static async updateUser(token, { firstname, lastname, style, medium }) {
		var res = await request(app).put("/users").send({
			firstname,
			lastname
		})
        .set("Authorization", "bearer " + token);
    ...
    {
        var r = await request(app).get("/users/self")
        .set("Authorization", "bearer " + token);
    ...
    static async createEvent(token, {name, description, dateStart, medium, longitude, latitude}) 
    {
        var res = await request(app)
        .post("/events")
        .attach("file", "./tests/img_test.jpg")
    ...
    static async createListEvent(token, listData)
    {
        ...
        return await Promise.all(listData.map(async (data) => await this.createEvent(token, data)));
    }
