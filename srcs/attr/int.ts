...
class IntAttribute 
{
    ...
    _check(newValue : any)
    {
        ...
        logger.debug(Number.isInteger(newValue), typeof newValue, parseInt(newValue))
...
class MiddlewareIntParser 
{
    ...
    getParser() 
    {
        return (req, res, next) => {
            ...
            if (errors.length > 0)
                return res.status(400).json({error: errors.map(error => error.name + ": " + error.value.error!)});
