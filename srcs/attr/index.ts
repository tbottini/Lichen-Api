...
class IndexAttr {
  ...
  _getContext(idObject) {
    ...
    if (this.id_array == null) {
      context =
        ' ' +
        this.name_array +
        ` = (select ${this.name_array} from ${this.table_name} where id = ${idObject})`
    } else {
  ...
  async uncrementOver(id: number) {
    logger.debug(
      this.produceSettingSql(
        'index - 1',
        `index > (select index from ${this.table_name} where id = ${id})`,
        id
      )
    )
  ...
  async reinit() {
    ...
    const get = 'select id , ' + this.name_array + ' from ' + this.table_name
    ...
    var querys = items.map(item => {
      const set =
        'update ' +
        this.table_name +
        ` set index = (select count(*) from ${this.table_name} where id < ${
          item.id
        } and ${this.name_array} = ${
