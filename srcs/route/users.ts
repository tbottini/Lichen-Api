...
router
	...
	.post("/register", async (req, res) => {
		...
		if (!passwordCorrect || !emailCorrect)
			return res
				.status(400)
				.json({ error: "email or password bad format" });
	...
	.put("/", [jwt.middleware, fileMiddleware()], async (req, res) => {
		...
		if (geoReferencedWrapper.error)
			return res.status(400).json({error: "geoReferenced : " + geoReferenced.error});
	...
	.post(
		...
		async (req, res) => {
			...
			if (result)
				return res.json({ msg: "you already follow this user" });
...
function reinjectUserFollow(user) {
	...
	user.likes = user.likes.filter((like) => like.artwork != null).map((like) => {
		logger.debug(like);
