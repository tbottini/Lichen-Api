...
router
	 ...
	 * @security jwt
	 */
	.post("/", [jwt.middleware, fileMiddleware(), positionParser.middleware], async (req, res) => {
		...
		if (dateStartAttr.error)
			return res
				.status(400)
				.json({ error: "bad format for date start attr" });

		const dateEndAttr = new DateAttr(dateEnd);
		if (dateEndAttr.error)
			return res
				.status(400)
				.json({ error: "bad format for date end attr" });
	...
	.get("/", parserQuery(querySearch), async (req, res) => {
		...
		var whereClause : Prisma.EventWhereInput = {
			...
			medium: (medium == undefined) ? undefined : {
				in: medium
			}};
		if (zone != undefined)
	...
	.put(
		"/:id",
		[parserMiddleware({ id: "int" }), jwt.middleware, fileMiddleware(), positionParser.middleware ],
		async (req, res) => {
			const { name, description, dateStart, dateEnd, index, medium} =
				req.body;
	...
	.get("/:id", parserMiddleware({ id: "int" }), async (req, res) => {
		...
		if (!result)
			return res.status(404).json({ error: "no event at this id" });
